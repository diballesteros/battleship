...
class Game extends Component {
  ...
  getAdjacentSquare() {
    ...
    if (this.state.lastSuccessfulMoves.length >= 2) {
      if (
        this.state.lastSuccessfulMoves[0] + 1 === this.state.lastSuccessfulMoves[1] ||
        this.state.lastSuccessfulMoves[0] - 1 === this.state.lastSuccessfulMoves[1]
      ) {
  ...
  getAdjacentHorizontalSquare(i) {
    if (
      this.state.computerMoves.includes(this.state.lastSuccessfulMoves[i] - 1) &&
      !FIRSTCOLUMNSQUARES.includes(this.state.lastSuccessfulMoves[i])
    ) {
      return this.state.lastSuccessfulMoves[i] - 1;
    } else if (
      this.state.computerMoves.includes(this.state.lastSuccessfulMoves[i] + 1) &&
      !LASTCOLUMNSQUARES.includes(this.state.lastSuccessfulMoves[i])
    ) {
      return this.state.lastSuccessfulMoves[i] + 1;
    }
  }
  ...
  receivePlayerAttack(shipId, playerMove) {
    ...
    if (!_.isNull(shipId)) {
      modifiedComputerShips = this.resolveBoardState(
        playerMove,
        shipId,
        this.state.computerShips.slice(),
        false
      );
    }
      ...
      <div className="game-page">
        <div className="game-view">
          ...
          {this.state.playerShips.length === 5 ? (
            <PlayerView
              receivePlayerAttack={(shipId, playerMove) =>
                this.receivePlayerAttack(shipId, playerMove)
              }
            ...
            <ShipFactory
              setShips={(builtShips, builtComputerShips) =>
                this.setShips(builtShips, builtComputerShips)
              }
            />
          )}
        </div>
        {this.state.gameResolved === true ? (
          <Modal modalText={'Game over'} modalFn={() => this.resetGame()}>
            New Game
          </Modal>
        ) : null}
      </div>
